...
w.Header().Set("Content-Type", "application/json")
   ...
   for rows.Next() {
    ...
    json.NewEncoder(os.Stdout).Encode(validation.Format)
  ...
  //  enco := json.NewEncoder(w)
    ...
    json.NewEncoder(os.Stdout).Encode(validation.Regx)


    input1 := []string{validation.Format}
    input2 := []string{validation.Regx}
